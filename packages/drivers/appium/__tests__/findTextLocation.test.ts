...
describe("findTextLocation", () => {
    ...
    it("should return scaled coordinates for found text", async () => {
        ...
        (driver.getWindowSize as jest.Mock).mockResolvedValue({ width: 1000, height: 800 });
    ...
    it("should throw if screenshot fails", async () => {
        ...
        await expect(findTextLocation("Fail")).rejects.toThrow("Failed to capture screenshot");
    });
});
