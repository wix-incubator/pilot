...
export class AutoPerformerPromptCreator {
    ...
    createPrompt(
        ...
        const reviewTypesArray = reviewTypes ? breakReviewArrayToItsTypes(reviewTypes) : [];
        if (reviewTypesArray.length === 0) {
            ...
            return [
                ...
                this.createInstructions(goal, isSnapshotImageAttached, reviewTypesArray),
            ]
                .flat()
                .join("\n");
    ...
    private createBasePrompt(reviewTypesArray?: string[]): string[] {
        ...
        prompt.push("", "Please adhere to the instructions below to provide detailed and helpful responses.", "");
    ...
    private createInstructions(
        ...
        return [
            ...
            "",
            ...this.createStepByStepInstructions(isSnapshotImageAttached, reviewTypesArray).map(
...
${reviewTypesArray? this.generateReviewSections(reviewTypesArray) : ""}`,
    ...
    private createStepByStepInstructions(
        ...
        if (reviewTypesArray) {
            const reviewTypes = reviewTypesArray.join(", ");
            steps.push(`Also, provide comprehensive overall ${reviewTypes} reviews with total scores, given all the screens seen in previous steps, inside the respective review sections.`,
