...
import {
    ...
    AutoReport,
    AutoReview,
    AutoReviewSection, AutoReviewSectionType,
    AutoStepPlan,
    AutoStepReport,
} from "@/types/auto";
import {
...
import {
    extractTaggedOutputs, extractTaggedOutputsController,
    OUTPUTS_MAPPINGS,
...
export class AutoPerformer {
    ...
    async analyseScreenAndCreatePilotStep(
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            try {
                ...
                const outputs = extractTaggedOutputsController(promptResult, reviewSectionTypes ? reviewSectionTypes : undefined);
                ...
                if (reviewSectionTypes && reviewSectionTypes.length > 0) {
                    reviewSectionTypes.forEach(reviewType => {
                        ...
                        if (reviewContent) {
                            review[reviewType.title] = this.extractReviewOutput(reviewContent);
                        }
                    });
                }
                ...
                if (hasReviews) {
                    ...
                    Object.keys(review).forEach(reviewType => {
                        if (review[reviewType]) {
                            const reviewTypeConfig = reviewSectionTypes?.find(rt =>
                                rt.title.toLowerCase() === reviewType.toLowerCase()
                            );
    ...
    async perform(goal: string, reviewSectionTypes?: AutoReviewSectionType[]): Promise<AutoReport> {
        const maxSteps = 100;
